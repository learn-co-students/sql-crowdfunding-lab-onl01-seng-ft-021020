...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, sum(pledges.amount) from pledges left outer join users on pledges.user_id = users.id group by users.name order by sum(pledges.amount);"
end
...
def selects_the_category_name_and_the_sum_total_of_the_all_its_pledges_for_the_books_category
  "Select projects.category, sum(pledges.amount) from pledges left outer join projects on pledges.project_id = projects.id where projects.category = 'books';"
end
